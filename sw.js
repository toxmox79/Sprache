const CACHE="bb-languages-v16-full-restore";
...
async function networkFirst(req){const c=await caches.open(CACHE);try{const r=await fetch(req,{cache:"no-store"});if(r&&r.ok)c.put(req,r.clone());return r}catch(e){return(await c.match(req))||(await c.match("./index.html"))||Response.error()}}
async function cacheFirst(req){const c=await caches.open(CACHE),hit=await c.match(req);if(hit)return hit;try{const r=await fetch(req);if(r&&r.ok)c.put(req,r.clone());return r}catch(e){return Response.error()}}
